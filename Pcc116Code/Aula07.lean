  ...
  lemma Perm_inv (xs ys : List A)
    : xs ~ ys →
      ...
      case perm_nil =>
        left
        simp
      case perm_skip x xs' ys' H1  =>
        right
        left
        exists x
        exists xs'
        exists ys'
      case perm_swap x y zs' =>
        right
        right
        left
        exists y
...
/- induction xs with  -/
    ...
    /-   intros H -/
    /-   simp [insert1] -/
    /-   rcases (Nat.decLe x y) with np | p  -/
    /-   ·  -/
    /-     simp  -/
    /-     cases H with  -/
    /-     | SortedSingle a =>  -/
    ...
    /-       omega  -/
    /-       constructor  -/
    /-     | SortedCons a b bs Hab Hs =>  -/
    /-       simp [insert1] at *  -/
    /-       revert IHys  -/
    /-       rcases (Nat.decLe x b) with Hxb | Hxb  -/
    /-       ·  -/
    /-         simp at * -/
    /-         intros H1  -/
    /-         constructor  -/
...
-- lemma isort_sorts (xs : List ℕ)
  ...
  /-   exact IHxs  -/
   ...
   /- induction xs with  -/
    ...
    /-   | isFalse H1 =>  -/
    /-     simp -/
    /-     have H2 : (y :: x :: ys) ~ (y :: insert1 x ys) := by -/
    /-      apply Permutation.perm_skip -/
    /-      assumption   -/
    /-     have H3 : (x :: y :: ys) ~ (y :: x :: ys) := by  -/
    /-       apply Permutation.perm_swap  -/
    /-     apply Permutation.perm_trans <;> assumption  -/
    /-   | isTrue H1 =>  -/
    /-     simp  -/
    /-     apply perm_refl  -/
