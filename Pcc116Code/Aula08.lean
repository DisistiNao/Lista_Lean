...
def Finite.size (t : Finite) : ℕ :=
  match t with
  | .unit => 1
  | .either t1 t2 => Finite.size t1 + Finite.size t2
  | .pair t1 t2 => Finite.size t1 * Finite.size t2

-- Defina uma função que teste a igualdade valores
-- definidos utilizando o universo U

def U.beq (t : U)(x y : t.asType) : Bool :=
  match t, x, y with
  | .bit, x, y => x = y
  | .char, x, y => x = y
  | .nat, x, y => x = y
  | .vec _u 0, Vec.nil, Vec.nil => true
  | .vec u (n' + 1), Vec.cons x xs, Vec.cons y ys =>
    U.beq u x y && U.beq (.vec u n') xs ys
